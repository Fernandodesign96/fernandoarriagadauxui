const translations = {
    es: {
        nav: {
            projects: 'Proyectos',
            about: 'Sobre Mí',
            process: 'Metodología',
            skills: 'Herramientas',
            contact: 'Contacto'
        },
        hero: {
            title: 'Diseño productos digitales end-to-end que convierten objetivos de negocio en resultados medibles',
            subtitle: 'Diseño de producto enfocado en conversión, performance y decisiones basadas en datos',
            cta: {
                primary: 'Ver proyectos',
                secondary: 'Hablemos'
            }
        },
        work: {
            title: 'Proyectos Seleccionados',
            intro: 'Casos de estudio que demuestran resolución de problemas, proceso y impacto medible en negocios reales.',
            viewStudy: 'Ver caso de estudio'
        },
        process: {
            title: 'Mi metodología de trabajo end-to-end',
            intro: 'Un enfoque estructurado que conecta investigación, diseño y métricas de negocio.',
            steps: {
                research: {
                    title: '01 · Research & Análisis',
                    description: 'Investigo problemas reales de usuarios y negocio mediante research cualitativo, análisis de datos y auditorías UX, identificando oportunidades accionables.'
                },
                strategy: {
                    title: '02 · Estrategia UX & Arquitectura',
                    description: 'Convierto insights en decisiones estratégicas: arquitectura de información, flujos críticos y definición de hipótesis alineadas a objetivos de negocio.'
                },
                design: {
                    title: '03 · Diseño & Prototipado',
                    description: 'Diseño soluciones funcionales y escalables, creando prototipos de alta fidelidad enfocados en usabilidad, accesibilidad y eficiencia.'
                },
                testing: {
                    title: '04 · Testing & Optimización',
                    description: 'Valido y optimizo mediante testing de usabilidad, A/B testing y análisis de métricas para mejorar conversión y performance del producto.'
                }
            }
        },
        skills: {
            title: 'Herramientas & Tecnologías',
            subtitle: 'Stack tecnológico para diseño, prototipado, investigación y desarrollo',
            filters: {
                all: 'Todo',
                strategy: 'Estrategia & Research',
                design: 'Diseño & Prototipado',
                dev: 'Desarrollo & Implementación',
                ai: 'Productividad & IA'
            },
            levels: {
                master: 'Maestro',
                advanced: 'Avanzado',
                intermediate: 'Intermedio'
            }
        },
        about: {
            title: 'Sobre Mí',
            subtitle: 'Diseño de Producto · Conversión · Datos',
            profile: {
                p1: 'Diseñador UX/UI con 5 años de experiencia en diseño, optimización y desarrollo de productos digitales. Especializado en Diseño Centrado en el Usuario, optimización de conversión, performance y métricas, aplicando metodologías como Design Thinking y UX Sprint.',
                p2: 'Experiencia en proyectos B2B, Fintech, e-commerce y productos digitales, liderando procesos end-to-end, desde la propuesta de valor hasta testeo UX.',
                p3: 'Manejo de herramientas de prototipado, análisis de datos y Front-End (HTML, CSS y Bootstrap), integrando IA aplicada al diseño y maquetación para agilizar procesos.'
            },
            interests: {
                research: 'Investigación',
                innovation: 'Innovación',
                technology: 'Tecnología',
                creativity: 'Creatividad',
                gamer: 'Gamer',
                tennis: 'Tenis',
                boxing: 'Boxeo',
                chess: 'Ajedrez'
            },
            cta: {
                contact: 'Contactarme',
                cv: 'Descargar CV'
            },
            cards: {
                experience: {
                    title: 'Especialización',
                    items: ['B2B SaaS', 'Fintech', 'E-commerce']
                },
                workflow: {
                    title: 'Enfoque',
                    items: ['Conversión', 'Performance', 'Diseño basado en métricas']
                },
                value: {
                    title: 'Herramientas clave',
                    items: ['Figma · Research', 'Tailwind · React · Vite', 'IA aplicada']
                }
            }
        },
        contact: {
            title: '¿Necesitas un diseñador UX/UI que impacte resultados de negocio?',
            subtitle: 'Hablemos sobre tu próximo producto o desafío UX.',
            infoTitle: 'Información de contacto',
            formTitle: 'Envía un mensaje',
            labels: {
                name: 'Tu nombre',
                email: 'Tu email',
                message: 'Tu mensaje'
            },
            cta: 'Enviar Mensaje',
            status: {
                sending: 'Enviando mensaje...',
                success: 'Mensaje enviado correctamente. Te responderé pronto.',
                error: 'Ocurrió un error. Intenta nuevamente.'
            },
            location: 'Santiago, Chile',
            links: {
                linkedin: 'LinkedIn',
                github: 'GitHub',
                cv: 'CV'
            }
        },
        footer: {
            rights: '© 2026 Fernando Arriagada. Todos los derechos reservados.',
            role: 'Senior UX/UI Designer · Product & Conversion Focused'
        }
    },
    en: {
        nav: {
            projects: 'Projects',
            about: 'About',
            process: 'Methodology',
            skills: 'Skills',
            contact: 'Contact'
        },
        hero: {
            title: 'End-to-end digital product design that converts business goals into measurable results',
            subtitle: 'Product design focused on conversion, performance, and data-driven decisions',
            cta: {
                primary: 'View work',
                secondary: 'Let\'s talk'
            }
        },
        work: {
            title: 'Selected Work',
            intro: 'Case studies demonstrating problem-solving, process, and measurable impact on real businesses.',
            viewStudy: 'View case study'
        },
        process: {
            title: 'My end-to-end methodology',
            intro: 'A structured approach that connects research, design, and business metrics.',
            steps: {
                research: {
                    title: '01 · Research & Analysis',
                    description: 'I research real user and business problems through qualitative research, data analysis, and UX audits, identifying actionable opportunities.'
                },
                strategy: {
                    title: '02 · UX Strategy & Architecture',
                    description: 'I convert insights into strategic decisions: information architecture, critical flows, and hypothesis definition aligned with business objectives.'
                },
                design: {
                    title: '03 · Design & Prototyping',
                    description: 'I design functional and scalable solutions, creating high-fidelity prototypes focused on usability, accessibility, and efficiency.'
                },
                testing: {
                    title: '04 · Testing & Optimization',
                    description: 'I validate and optimize through usability testing, A/B testing, and metric analysis to improve product conversion and performance.'
                }
            }
        },
        skills: {
            title: 'Tools & Technologies',
            subtitle: 'Technology stack for design, prototyping, research, and development',
            filters: {
                all: 'All',
                strategy: 'Strategy & Research',
                design: 'Design & Prototyping',
                dev: 'Development & Implementation',
                ai: 'Productivity & AI'
            },
            levels: {
                master: 'Master',
                advanced: 'Advanced',
                intermediate: 'Intermediate'
            }
        },
        about: {
            title: 'About Me',
            subtitle: 'Product Design · Conversion · Data',
            profile: {
                p1: 'UX/UI Designer with 5 years of experience in design, optimization, and development of digital products. Specialized in User-Centered Design, conversion optimization, performance, and metrics, applying methodologies like Design Thinking and UX Sprint.',
                p2: 'Experience in B2B, Fintech, e-commerce, and digital product projects, leading end-to-end processes from value proposition to UX testing.',
                p3: 'Proficiency in prototyping tools, data analysis, and Front-End (HTML, CSS, and Bootstrap), integrating AI applied to design and layout to streamline processes.'
            },
            interests: {
                research: 'Research',
                innovation: 'Innovation',
                technology: 'Technology',
                creativity: 'Creativity',
                gamer: 'Gamer',
                tennis: 'Tennis',
                boxing: 'Boxing',
                chess: 'Chess'
            },
            cta: {
                contact: 'Get in Touch',
                cv: 'Download CV'
            },
            cards: {
                experience: {
                    title: 'Specialization',
                    items: ['B2B SaaS', 'Fintech', 'E-commerce']
                },
                workflow: {
                    title: 'Focus',
                    items: ['Conversion', 'Performance', 'Metric-based design']
                },
                value: {
                    title: 'Key Tools',
                    items: ['Figma · Research', 'Tailwind · React · Vite', 'Applied AI']
                }
            }
        },
        contact: {
            title: 'Need a UX/UI designer who impacts business results?',
            subtitle: 'Let\'s talk about your next product or UX challenge.',
            infoTitle: 'Contact Information',
            formTitle: 'Send a message',
            labels: {
                name: 'Your name',
                email: 'Your email',
                message: 'Your message'
            },
            cta: 'Send Message',
            status: {
                sending: 'Sending message...',
                success: 'Message sent successfully. I will get back to you soon.',
                error: 'An error occurred. Please try again.'
            },
            location: 'Santiago, Chile',
            links: {
                linkedin: 'LinkedIn',
                github: 'GitHub',
                cv: 'CV'
            }
        },
        footer: {
            rights: '© 2026 Fernando Arriagada. All rights reserved.',
            role: 'Senior UX/UI Designer · Product & Conversion Focused'
        }
    }
}

export default translations
